...
public class SnakeGameView extends View implements SensorEventListener {
    ...
    public SnakeGameView(Context context, @Nullable AttributeSet attrs) {
        ...
        snakePaint.setColor(Color.MAGENTA);

        snakeHead = ContextCompat.getDrawable(context,R.drawable.snakehead);
        planet = ContextCompat.getDrawable(context,R.drawable.planet);
        asteroid = ContextCompat.getDrawable(context,R.drawable.asteroid);
    ...
    public void setDifficulty(int difficulty) {
        level = difficulty;
        difficulty ++;
        snakeGame.setInitialSpeed(difficulty * 2.0);
        ...
        snakeGame.setWallPlacementProbability(snakeGame.getWallPlacementProbability() * difficulty);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (snakeGame.update())
        {
            ...
            drawDrawable(snakeHead, canvas, head, Snake.BODY_PIECE_SIZE_DP);
            canvas.restore();

            for (PointF w: snakeGame.getWallLocations())
            {
                drawDrawable(asteroid, canvas, w, SnakeGame.WALL_SIZE_DP);
            }

            PointF foodLocation = snakeGame.getFoodLocation();
            drawDrawable(planet, canvas, foodLocation, SnakeGame.FOOD_SIZE_DP);
        }
        else
        {
            canvas.drawText("Score: " + snakeGame.getScore(), spToPx(displayMetrics.widthPixels / 4f), spToPx(20.0f),  scorePaint);
